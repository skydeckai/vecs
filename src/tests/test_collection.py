...
def test_upsert(client: vecs.Client) -> None:
    ...
    new_record = ("vec0", np.zeros(384), {}, "text", 0, 0, 2, 3, 0, 0)
    movies.upsert([new_record])
    db_record = movies["vec0"]
    db_record[0] == new_record[0]
    db_record[1] == new_record[1]
    db_record[2] == new_record[2]
    db_record[3] == new_record[3]
    db_record[4] == new_record[4]
    db_record[5] == new_record[5]
...
def test_getitem(client: vecs.Client) -> None:
    ...
    movies.upsert(records=[("1", [1, 2, 3], {}, "1", 1, 0, 3, 4, 5, 6)])

    assert movies["1"] is not None
    assert len(movies["1"]) == 10
...
def test_query(client: vecs.Client) -> None:
    ...
    (
        ...
        query_meta,
        query_text,
        query_doc_instance_id,
        query_order,
        query_memento_membership,
        query_app_id,
        query_begin_offset_byte,
        query_chunk_bytes,
    ) = bar["vec5"]
    ...
    assert res[0][4] == query_memento_membership
    assert res[0][5] == query_app_id
    assert res[0][6] == query_begin_offset_byte
    assert res[0][7] == query_chunk_bytes
    ...
    assert res[0][6] == query_app_id
    assert res[0][7] == query_begin_offset_byte
    assert res[0][8] == query_chunk_bytes
    assert res[0][9] == query_text
...
def test_query_filters(client: vecs.Client) -> None:
    ...
    records = [
        ("0", [0, 0, 0, 0], {"year": 1990}, "0", 1, 0, 3, 4, 5, 6),
        ("1", [1, 0, 0, 0], {"year": 1995}, "1", 1, 0, 3, 4, 5, 6),
        ("2", [1, 1, 0, 0], {"year": 2005}, "2", 1, 0, 3, 4, 5, 6),
        ("3", [1, 1, 1, 0], {"year": 2001}, "3", 1, 0, 3, 4, 5, 6),
        ("4", [1, 1, 1, 1], {"year": 1985}, "4", 1, 0, 3, 4, 5, 6),
        ("5", [2, 1, 1, 1], {"year": 1863}, "5", 1, 0, 3, 4, 5, 6),
        ("6", [2, 2, 1, 1], {"year": 2021}, "6", 1, 0, 3, 4, 5, 6),
        ("7", [2, 2, 2, 1], {"year": 2019}, "7", 1, 0, 3, 4, 5, 6),
        ("8", [2, 2, 2, 2], {"year": 2003}, "8", 1, 0, 3, 4, 5, 6),
        ("9", [3, 2, 2, 2], {"year": 1997}, "9", 1, 0, 3, 4, 5, 6),
    ]
...
def test_filters_eq(client: vecs.Client) -> None:
    ...
    records = [
        ("0", [0, 0, 0, 0], {"a": 1}, "0", 1, 0, 3, 4, 5, 6),
        ("1", [1, 0, 0, 0], {"a": 2}, "1", 1, 0, 3, 4, 5, 6),
        ("2", [1, 1, 0, 0], {"a": 3}, "2", 1, 0, 3, 4, 5, 6),
        ("3", [1, 1, 1, 0], {"b": [1, 2]}, "3", 1, 0, 3, 4, 5, 6),
        ("4", [1, 1, 1, 1], {"b": [1, 3]}, "4", 1, 0, 3, 4, 5, 6),
        ("5", [1, 1, 1, 1], {"b": 1}, "5", 1, 0, 3, 4, 5, 6),
        ("6", [1, 1, 1, 1], {"c": {"d": "hi"}}, "6", 1, 0, 3, 4, 5, 6),
    ]
...
def test_filters_in(client: vecs.Client) -> None:
    ...
    records = [
        ("0", [0, 0, 0, 0], {"a": 1, "b": 2}, "0", 1, 0, 3, 4, 5, 6),
        ("1", [1, 0, 0, 0], {"a": [1, 2, 3]}, "1", 1, 0, 3, 4, 5, 6),
        ("2", [1, 1, 0, 0], {"a": {"1": "2"}}, "2", 1, 0, 3, 4, 5, 6),
        ("3", [0, 0, 0, 0], {"a": "1"}, "3", 1, 0, 3, 4, 5, 6),
    ]
...
def test_ivfflat(client: vecs.Client) -> None:
    dim = 4
    bar = client.get_or_create_collection(name="bar", dimension=dim)
    bar.upsert([("a", [1, 2, 3, 4], {}, "a", 2, 4, 3, 4, 5, 6)])
...
def test_index_build_args(client: vecs.Client) -> None:
    dim = 4
    bar = client.get_or_create_collection(name="bar", dimension=dim)
    bar.upsert([("a", [1, 2, 3, 4], {}, "a", 1, 2, 3, 4, 5, 6)])
